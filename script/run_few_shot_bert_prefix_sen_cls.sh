...
python LM_data_filtering_sen_cls.py --output-path $OUTPUT_ROOT/clean_nlg_mix_tag_output.txt --data-path $OUTPUT_ROOT/nlg_model_mix_output_part1.txt $OUTPUT_ROOT/nlg_model_mix_output_part3.txt 
python LM_data_filtering_sen_cls.py --output-path $OUTPUT_ROOT/clean_nlg_mix_keyword_output.txt --data-path $OUTPUT_ROOT/nlg_model_mix_output_part2.txt $OUTPUT_ROOT/nlg_model_mix_output_part4.txt 			

# train NLU & NLG model only using few-shot training data
CUDA_VISIBLE_DEVICES=$GPUID python train_SenLabel.py --config $CONFIG_ROOT/bert_nlu.yml --config-override train_path $OUTPUT_ROOT/train_whole_$SHOT.txt random_seed $RANDOM_SEED --serialization-dir $OUTPUT_ROOT/bert_nlu_model_1 --train
for i in 1 2 3
do
	# NLU model run for consistency filtering
	CUDA_VISIBLE_DEVICES=$GPUID python train_SenLabel.py --config $CONFIG_ROOT/bert_nlu.yml --start-from-checkpoint $OUTPUT_ROOT/bert_nlu_model_$i --test \
	...
	CUDA_VISIBLE_DEVICES=$GPUID python train_SenLabel.py --config $CONFIG_ROOT/bert_nlu.yml --serialization-dir $OUTPUT_ROOT/bert_nlu_model_$((i+1)) --train \
